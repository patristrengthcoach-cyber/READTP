...
### Ficha de sesión
- **Filtro global de jugador**: en la parte superior de la página (visible en todas las pestañas) hay un selector con los jugadores de baja actuales (nombre + ID + categoría, tomados del seguimiento). Elegir uno filtra automáticamente la ficha, el registro de carga y el seguimiento por ese jugador, y **carga en la ficha su última sesión guardada** — nunca la de otro jugador. La fase, el objetivo y el siguiente número de sesión sugerido se actualizan siempre a partir de esa última sesión, tenga o no ejercicios guardados (por ejemplo, si se registró como "sesión esporádica" desde Registro y carga); los ejercicios en concreto solo se cargan cuando la última sesión los tiene. Esta comprobación se hace por sesión concreta, no solo por jugador: si aparece una sesión más reciente para el mismo jugador (por ejemplo, guardada por otra persona del cuerpo técnico, o al recargar la página) la ficha se actualiza sola en vez de quedarse con datos antiguos, y nunca borra un trabajo en curso sin guardar si no hay nada nuevo que cargar. Los jugadores dados de alta no aparecen en este selector (ver más abajo).
...
- **Ficha de sesión** en formato tabla (ID · Ejercicio · Series · Repeticiones · Carga/Intensidad · RPE · Descanso · Notas), editable en línea, reordenable, con minutos por bloque, número de sesión junto a la fecha, e imprimible. Al descargar/imprimir, **todo el contenido de la ficha se ajusta automáticamente para caber en una sola hoja** (igual que en el informe de seguimiento), reduciendo el tamaño si hace falta en vez de cortar información entre páginas.
...
- **Resumen y carga de la sesión**: duración total, RPE de la sesión y carga (session-RPE = duración × RPE) en vivo, dolor EVA post-sesión, cumplimiento y observaciones. Cada sesión guardada incluye **todos sus ejercicios**, no solo el resumen — se puede reutilizar más tarde como plantilla (ver *Memoria de sesiones*).

### Registro y carga
- Guarda cada sesión ejecutada por jugador (incluye nº de sesión, objetivo, ejercicios completos, tests) y calcula **carga aguda (7 días)**, **carga crónica (media semanal de 4 semanas)**, **ACWR**, **monotonía** y **strain**, con el mismo criterio del Excel (ACWR 0.8–1.3 óptimo, >1.5 riesgo alto), más un mini-gráfico de carga diaria.
...
- **+ Añadir sesión realizada**: para una sesión puntual que se hizo sin pasar por la ficha (algo esporádico o de última hora). Un formulario rápido con fecha, nº, fase, duración, RPE, dolor EVA, cumplimiento y observaciones — sin necesidad de construir ejercicios.
...
- **Filtro Todos / Altas / Bajas**: tres pastillas que controlan qué jugadores aparecen en la lista de selección para el informe (por defecto, solo los de baja).
- **Selección múltiple para el informe**: dentro del filtro elegido, una lista de casillas permite marcar uno, varios o todos los jugadores que se quieren incluir en el informe (con botones "Marcar todos" / "Ninguno"). Al llegar un jugador nuevo por sincronización se marca automáticamente.
- **Fase editable y que se actualiza sola**: cada vez que se guarda una sesión (desde la Ficha o como sesión rápida) la fase de ese jugador en el seguimiento se actualiza automáticamente a la de esa sesión. También se puede corregir a mano en cualquier momento desde el lápiz (✎ → "Fase actual") — ese cambio se refleja al instante en la tabla, y se mantiene hasta que se guarde una sesión nueva.
...
- **Los ejercicios de cada sesión se ven directamente en la tabla**: debajo de cada fila aparece un resumen ("Ejercicios guardados: Gym: Nordic Hamstring…, Calent.: Movilidad articular…") o, si esa sesión no tiene ninguno guardado (por ejemplo, una "sesión esporádica" añadida desde Registro y carga), un aviso claro de que está vacía — sin tener que adivinar ni hacer clic en nada.
- **→ Ficha**: botón disponible en **todas** las sesiones guardadas (tengan o no ejercicios). Escribe esa sesión directamente en la Ficha de sesión — fase, objetivo y el siguiente número sugerido siempre, más los ejercicios de cada bloque si esa sesión los tiene. Se puede aplicar al mismo jugador o cambiar el filtro global a otro distinto antes de guardar; lo escrito no se pierde al cambiar de jugador (aparece un aviso "Plantilla cargada" con opción de quitarla).
